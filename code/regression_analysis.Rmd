...
```{r Read in Data}
#Inputs data
pest_fuel_use_fert_emis <- read_csv("raw_data/faostat/pesticide_and_fuel_use_and_fertilizer_emissions.csv")
...
syn_N_use <-read_csv("raw_data/faostat/fertilizer_use.csv")
...
Yield <- prod[, c(1,2,5)]
Cropland <- prod[, c(1,2,4)]
Production <- prod[,c(1,2,3)]
```
...
reg <- spread(pest_fuel_use_fert_emis, key = Item, value = Value) #convert data to wide format 
reg <- inner_join(reg, prod, by = c("Country" = "country", "Year" = "year")) %>% inner_join(syn_N_use) 

#Determine number of negative and 0 values for inputs
dat <- reg
nrow(reg[reg$`Energy used in fishery`<= 0, ]) #7955 obs
nrow(reg[reg$`Nutrient nitrogen N (total)` <= 0, ]) #7228 obs
nrow(reg[reg$`Pesticides (total)` <= 0, ]) #5611 obs
nrow(reg[reg$`Synthetic Nitrogen fertilizers` <= 0, ]) #1674 obs, different from Dan's number of 1002
nrow(reg[reg$`Total Energy` <= 0, ]) #5459 obs, different from Dan's number of 168
nrow(reg[reg$`Transport fuel used in agriculture (excl. fishery)` <= 0, ]) #6654
nrow(reg[reg$Syn_N_use <= 0, ]) #1019

#Replace negative and 0 values with NA
reg[reg <= 0] <- NA #change to less than or equal to remove negative values for transportation fuel
...
reg <-  reg %>% group_by(Country) %>% mutate(`Energy used in fishery` = na.locf(`Energy used in fishery`, na.rm = FALSE))
reg <-  reg %>% group_by(Country) %>% mutate(`Nutrient nitrogen N (total)` = na.locf(`Nutrient nitrogen N (total)`, na.rm = FALSE))
reg <-  reg %>% group_by(Country) %>% mutate(`Pesticides (total)` = na.locf(`Pesticides (total)`, na.rm = FALSE))
reg <-  reg %>% group_by(Country) %>% mutate(`Synthetic Nitrogen fertilizers` = na.locf(`Synthetic Nitrogen fertilizers`, na.rm = FALSE))
reg <-  reg %>% group_by(Country) %>% mutate(`Total Energy` = na.locf(`Total Energy`, na.rm = FALSE))
reg <-  reg %>% group_by(Country) %>% mutate(`Transport fuel used in agriculture (excl. fishery)` = na.locf(`Transport fuel used in agriculture (excl. fishery)`, na.rm = FALSE))
reg <-  reg %>% group_by(Country) %>% mutate(`Production (kcal)` = na.locf(`Production (kcal)`, na.rm = FALSE))
reg <-  reg %>% group_by(Country) %>% mutate(`Cropland (ha)` = na.locf(`Cropland (ha)`, na.rm = FALSE))
reg <-  reg %>% group_by(Country) %>% mutate(Syn_N_use = na.locf(Syn_N_use, na.rm = FALSE))
