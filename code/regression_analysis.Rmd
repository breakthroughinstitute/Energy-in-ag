...
nrow(reg[reg$`Energy used in fishery`<= 0, ]) 
nrow(reg[reg$`Nutrient nitrogen N (total)` <= 0, ]) 
nrow(reg[reg$`Pesticides (total)` <= 0, ]) 
nrow(reg[reg$`Synthetic Nitrogen fertilizers` <= 0, ]) 
nrow(reg[reg$`Total Energy` <= 0, ]) 
nrow(reg[reg$`Transport fuel used in agriculture (excl. fishery)` <= 0, ])
nrow(reg[reg$Syn_N_use <= 0, ])
...
reg <-  reg %>% group_by(Country) %>% mutate(tot_kcal = na.locf(tot_kcal, na.rm = FALSE))
reg <-  reg %>% group_by(Country) %>% mutate(cropland = na.locf(cropland, na.rm = FALSE))
reg <-  reg %>% group_by(Country) %>% mutate(kcal_ha = na.locf(kcal_ha, na.rm = FALSE))
reg <-  reg %>% group_by(Country) %>% mutate(Syn_N_use = na.locf(Syn_N_use, na.rm = FALSE))

#Calculate total fuel applied from on farm sources, subtract emissions from fisheries and transportation, ignoring NAs in fisheries and transportation but not in total energy
reg$Fuel.emissions <- ifelse(!is.na(reg$`Total Energy`), reg$`Total Energy` - rowSums(reg[,c("Energy used in fishery","Transport fuel used in agriculture (excl. fishery)")], na.rm=TRUE), NA)

#Cropland units are per 1000 ha, so multiply cropland values by 1000
reg$cropland <- reg$cropland * 1000 

#Calculate each emissions or input use per ha 
reg$Fuel_ha <-reg$Fuel.emissions/reg$cropland #units are gigagrams/ha
reg$P_ha <- reg$`Pesticides (total)`/reg$cropland #units are tons applied/ha
reg$synN_use_ha <- reg$Syn_N_use/reg$cropland #units are kg applied/ha       
reg$SynN_ha <- reg$`Synthetic Nitrogen fertilizers`/reg$cropland#units are gigagrams/ha
