...
energy_emiss <- read_csv("raw_data/faostat/energy_use_emissions.csv") %>% 
  ...
  mutate(Value = coalesce(Value,0)) %>% #replace NA with 0. Avoids total energy emissions being NA when just one component is NA
  ...
  mutate(en_emiss = `Total Energy` - `Energy used in fishery`,  #calculate farm emissions as total - fishery 
