...
reg_W_aggregate <- aggregate_reg(reg_W, "West")
reg_C_aggregate <- aggregate_reg(reg_C, "Central")
reg_S_aggregate <- aggregate_reg(reg_S, "South")
reg_N_aggregate <- aggregate_reg(reg_N, "North")
```
...
reg_fe_results <- tibble(dep_var=character(), scenario = character(), region = character(), fit=double(), lwr = double(), upr = double(), se_log = double(), df =integer(), residual_scale = double())
...
reg_fe_results <- reg_fe_results %>% 
  ...
  mutate(across(starts_with("f_m"),  ~.x * arable_area * fert_ef * kg_to_Gg), #convert kg fertilizer use/ha to Gg CO2e emissions from manufacturing for all fert use
         across(starts_with("pest"),  ~.x * arable_area * pest_ef * tons_to_Gg), #convert tons pesticide use/ha to Gg CO2e emissions from manufacturing for all pesticide use
         across(starts_with("en"),  ~.x * arable_area), #convert Gg CO2e from energy use per ha  to Gg CO2 from total energy use
         across(starts_with("f_a"),  ~.x * arable_area), #convert Gg CO2e from fert application per ha to Gg CO2 from total application
         across(starts_with("m_"),  ~.x * arable_area * tpes_ef * tons_to_Gg) #convert from Terajoules per ha to gigagran Co2e from total machinery embodied energy
         )
```

```{r calculate 95% confidence interval for aggregated emissions}
...
all_emiss <- inner_join(reg_fe_results, luc_co2_region, by = c("scenario", "region"), relationship = "many-to-many") %>%  #join input and LUC emissions
...
```{r Fig 1. Total LUC and Input Emissions by Scenario}
fig_dat <- all_emiss %>% 
  mutate(scenario = recode(scenario, "b" = "Baseline", "p" = "Projected", "h" = "High"))  #Spell out scenarios for graphing and change order of scenario column so it is not in alphabetical order, but rather in the order of the yield-scenarios

c_loss_selected <- "H" #change to change which soil carbon loss scenario is used for following graphs
...
fig1 <- fig_dat %>% 
  ...
  select(scenario, region, estimate, luc_co2_high, tot_input_co2) %>% 
  pivot_wider(names_from = estimate, values_from = c(luc_co2_high, tot_input_co2)) %>% #restructure so there is a column for the lwr and upr input emission totals
  group_by(scenario) %>% 
  summarize(tot_luc = sum(tot_co2_high), tot_input = sum(tot_input_co2)) %>% 
  #left off here needing to find a way to aggregate the error bars properly across regions
