...
aggregate_reg <- function(region_data, region_name){
  ...
  output <- output %>% mutate(yield = tot_kcal/cropland, en_emiss_ha = en_emiss/cropland, p_ha = pest_use/cropland, fert_emiss_ha = fert_emiss/cropland, fert_use_per_ha = fert_use/cropland, country = region_name)
...
reg_other_country <- reg_other_country %>%
  dplyr::select(country,year, energy_used_in_fishery, synthetic_nitrogen_fertilizers,fert_use,  total_energy, transport_fuel_used_in_agriculture_excl_fishery,pesticides_total, fuel_emissions,en_emiss_ha, p_ha, fert_emiss_ha,fert_use_ha, yield,production_kcal, cropland_ha)

reg_all_countries <- dplyr::full_join(reg_other_country, reg_W_aggregate) 

reg_all_countries <- dplyr::full_join(reg_all_countries, reg_E_aggregate)  

reg_all_countries <- dplyr::full_join(reg_all_countries, reg_S_aggregate)

reg_all_countries <- dplyr::full_join(reg_all_countries , reg_C_aggregate)

reg_all_countries <- dplyr::full_join(reg_all_countries , reg_N_aggregate) 
