...
fuel.ols <- lm(filter_reg , formula = log(en_emiss_ha) ~ log(yield) + factor(country))

#Synthetic N emissions OLS Regression 
syn_n.ols <- lm(filter_reg, formula = log(fert_emiss_ha) ~ log(yield) + factor(country)) #no #outliers in boxplot

#Synthetic N use or application OLS regression
fert_use.ols <- lm(filter_reg, formula = log(fert_use_ha) ~ log(yield) + factor(country))

#Energy embodied in machinery OLS regression
m_energy.ols <- lm(filter_reg, formula = log(m_energy_ha) ~ log(yield) + factor(country))
```
...
```{r Remove? Import yields for each scenario and region to estumate input use and emissions}
...
var_models = tibble(var = c("en_emiss_ha", "fert_use_ha", "fert_emiss_ha", "p_ha", "m_energy_ha"),
                   model = c("fuel.ols", "fert_use.ols", "syn_n.ols", "p.ols", "m_energy.ols"))
...
```{r Delete? Create table of results - moved this code chunk to after machinery ols is calculated}
...
reg_fe_results <- reg_fe_results %>% 
  ...
  left_join(scenarios, by = c("region" = "country", "scenario")) %>% #add production scenario variables i.e. yield, area to fe regression results table. join leaves out total Africa region
  pivot_wider(names_from = dep_var, names_glue = "{dep_var}_{.value}", values_from = c(fit, lwr, upr)) %>% #make wide to multiple columns for fertilizer and pesticide use per ha by emission factors  
  mutate(across(starts_with("f_m"),  ~.x*harv_area*fert_ef*kg_to_Gg), #convert kg fertilizer use/ha to Gg CO2e emissions from manufacturing for all fert use
         ...
         across(starts_with("m_"),  ~.x*harv_area*tpes_ef*tons_to_Gg) #convert from Terajoules per ha to gigatons Co2e from total machinery embodied energy
         )
```
...
```{r Remove - Calculate emissions for each input source, region, and scenario}
# #calculate total input use 
# 
...
```{r Remove - Set up OLS for Machinery Emissions}
# 
# #Calculate Energy Embodied in Machinery Emissions 
# machinery_energy <- read_csv("Energy and GHG Analysis (2019)/Analysis/energy_ag_analysis/raw_data/Machinery_Emissions.csv")
...
```{r Remove - Calculate Machinery emissions}
...
```{r Remove -Add Machinery energy adn emissions predictions to energy predicted values table }
...
```{r Remove -  Pull out lower and upper confidence intervals from model predictions}
# 
# 
...
```{r Remove - Save model coeffs for each region}
