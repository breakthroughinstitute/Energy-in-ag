...
```{r Read in Data}
#Inputs data
reg_dat <- read_csv("~/Google Drive/My Drive/Food & Farming/Energy in ag/Energy and GHG Analysis (2019)/Analysis/energy_ag_analysis/raw_data/FAOSTAT_Regression_Dat_allyears.csv")
#FAOSTAT data is in long format, contains data on all countries and years on synthetic fertilizer emissons (Gg), pesticide application (tons) and energy use emissions (Gg)

syn_N_use <- read_csv("~/Google Drive/My Drive/Food & Farming/Energy in ag/Energy and GHG Analysis (2019)/Analysis/energy_ag_analysis/raw_data/FAOSTAT_synN_USE.csv")
#FAOSTAT data is in long format, contains info on all countries and years on synthetic fertilizer application (kg)

syn_N_use[syn_N_use == 0] <- NA #0's are likely missing data
colnames(syn_N_use) <- c("Country", "Year", "Syn_N_use")

#Yield and Cropland Data from calculate_calories_per_cropland.Rmd
dat1 <- read_csv(file = "~/Google Drive/My Drive/Food & Farming/Energy in ag/Energy and GHG Analysis (2019)/Analysis/energy_ag_analysis/clean_data/prod.cropland.csv")

#load data on dan machine
# setwd("/System/Volumes/Data/Volumes/GoogleDrive/My Drive/BTI Research/Food & Farming/Energy in ag/Energy and GHG Analysis (2019)/Analysis/energy_ag_analysis")
# reg_dat <- read_csv("raw_data/FAOSTAT_Regression_Dat_allyears.csv")
# syn_N_use <- read_csv("raw_data/FAOSTAT_synN_USE.csv")
# syn_N_use[syn_N_use == 0] <- NA #0's are likely missing data
# colnames(syn_N_use) <- c("Country", "Year", "Syn_N_use")
# dat1 <- read_csv(file = "clean_data/prod.cropland.csv") 

Yield <- dat1[, c(1,2,5)]
Cropland <- dat1[, c(1,2,4)]
Production <- dat1[,c(1,2,3)]
```


```{r Calculate total fuel emissions, calculate emissions per ha for each input }
#join dataframes on input use, production, and cropland
reg_temp <- spread(reg_dat, key = Item, value = Value) #convert data to wide format 
reg_temp_a <- reg_temp
reg_temp <- merge(reg_temp,Production, by = c("Country", "Year")) 
reg <- merge(reg_temp, Cropland)
reg <- merge(reg, syn_N_use)

#Determine number of negative and 0 values for inputs
dat <- reg
nrow(dat[dat$`Energy used in fishery`<= 0, ]) #7955 obs
nrow(dat[dat$`Nutrient nitrogen N (total)` <= 0, ]) #7228 obs
nrow(dat[dat$`Pesticides (total)` <= 0, ]) #5611 obs
nrow(dat[dat$`Synthetic Nitrogen fertilizers` <= 0, ]) #1674 obs, different from Dan's number of 1002
nrow(dat[dat$`Total Energy` <= 0, ]) #5459 obs, different from Dan's number of 168
nrow(dat[dat$`Transport fuel used in agriculture (excl. fishery)` <= 0, ]) #6654
nrow(dat[dat$Syn_N_use <= 0, ]) #1019

#Replace negative and 0 values with NA
dat[dat <= 0] <- NA #change to less than or equal to remove negative values for transportation fuel


#Fill in missing data for each column by country. Apply the last one carried forward method to each column in the dataframe (called dat below), grouping by country so that if the first observation is NA, the function doesn't replace with observation from different country. Will not fill in data for previous years.
dat <-  dat %>% group_by(Country) %>% mutate(`Energy used in fishery` = na.locf(`Energy used in fishery`, na.rm = FALSE))
dat <-  dat %>% group_by(Country) %>% mutate(`Nutrient nitrogen N (total)` = na.locf(`Nutrient nitrogen N (total)`, na.rm = FALSE))
dat <-  dat %>% group_by(Country) %>% mutate(`Pesticides (total)` = na.locf(`Pesticides (total)`, na.rm = FALSE))
dat <-  dat %>% group_by(Country) %>% mutate(`Synthetic Nitrogen fertilizers` = na.locf(`Synthetic Nitrogen fertilizers`, na.rm = FALSE))
dat <-  dat %>% group_by(Country) %>% mutate(`Total Energy` = na.locf(`Total Energy`, na.rm = FALSE))
dat <-  dat %>% group_by(Country) %>% mutate(`Transport fuel used in agriculture (excl. fishery)` = na.locf(`Transport fuel used in agriculture (excl. fishery)`, na.rm = FALSE))
dat <-  dat %>% group_by(Country) %>% mutate(`Production (kcal)` = na.locf(`Production (kcal)`, na.rm = FALSE))
dat <-  dat %>% group_by(Country) %>% mutate(`Cropland (ha)` = na.locf(`Cropland (ha)`, na.rm = FALSE))
dat <-  dat %>% group_by(Country) %>% mutate(Syn_N_use = na.locf(Syn_N_use, na.rm = FALSE))

reg <- dat

#Add a column called "val" which replaces the NAs with 0s to calculate fuel emissions without changing the NA data for the Total Energy, Energy Used in Fishery, and Transport fuel columns

for (i in 1:nrow(reg)) {
    val <-  reg$`Energy used in fishery`[i] 
    if (is.na(val) == "TRUE") {
       reg$val_EF[i]  <- 0
    } else {
      reg$val_EF[i]  <- val 
...
for (i in 1:nrow(reg)) {
    val <-  reg$`Transport fuel used in agriculture (excl. fishery)`[i] 
    if (is.na(val) == "TRUE") {
       reg$val_trans[i]  <- 0
    } else {
     reg$val_trans[i]  <- val 
...
#Calculate total fuel applied from on farm sources, subtract emissions from fisheries and transportation  
reg$Fuel.emissions <- reg$val_totE -  reg$val_EF -  reg$val_trans 

#Remove the val_EF, val_totE, and val_trans cols
reg$val_EF <- NULL
reg$val_totE <- NULL
reg$val_trans <- NULL

#Cropland units are per 1000 ha, so multiply cropland values by 1000
reg$`Cropland (ha)` <- reg$`Cropland (ha)` * 1000 

#Calculate each emissions or input use per ha 
reg$Fuel_ha <-reg$Fuel.emissions/reg$`Cropland (ha)` #units are gigagrams/ha
reg$P_ha <- reg$`Pesticides (total)`/reg$`Cropland (ha)` #units are tons applied/ha
reg$synN_use_ha <- reg$Syn_N_use/reg$`Cropland (ha)` #units are kg applied/ha       
reg$SynN_ha <- reg$`Synthetic Nitrogen fertilizers`/reg$`Cropland (ha)`#units are gigagrams/ha

#reg calc yield
reg$yield <-  reg$`Production (kcal)`/reg$`Cropland (ha)`
plot(reg$yield, reg$SynN_ha, main = "Yield vs. Syn N emissions per ha")
plot(reg$yield, reg$synN_use_ha, main = "Yield vs. Syn N yse per ha (manufacturing)")
plot(reg$yield, reg$Fuel_ha, main = "Yield vs. On farm fuel emissions per ha")
...
nrow(reg) #9650
sum(reg$production_kcal !=  0) #returns 9650 which is equivalent to number of obs in reg #dataframe
sum(reg$cropland_ha !=  0) #returns 9650 which is equivalent to number of obs in reg dataframe

print("number duplicated")
get_dupes(reg, country, year) # no duplicates found

#count fuel emissions per ha observations by country
count_by_country <- reg %>% 
dplyr::group_by(country) %>% 
 dplyr::summarize(count = sum(is.na(fuel_ha)==FALSE))
count_by_country
one.value <- unique(count_by_country$count)
one.value #if there is only 1 unique value in the count column then all countries have the #same number of obs, if countries have different numbers of observations, returns an #unbalanced panel dataset 

# count pesticide observations by country
count_by_country <-reg %>% 
  dplyr::group_by(country) %>% 
  dplyr::summarize(count = sum(is.na(p_ha)==FALSE))
count_by_country
one.value <- unique(count_by_country$count)
one.value #if there is only 1 unique value in the count column then all countries have the same number of obs, if countries have different numbers of observations, returns an unbalanced panel dataset 

# count synthetic N per hectare observations by country
count_by_country <-reg %>% 
  dplyr::group_by(country) %>% 
  dplyr::summarize(count = sum(is.na(syn_n_ha)==FALSE))
count_by_country
one.value <- unique(count_by_country$count) #if there is only 1 unique value in the count column then all countries have the same number of obs, unbalanced panel
...
plot(reg$production_kcal/reg$cropland_ha, reg$syn_n_use_ha) 
boxplot(reg$syn_n_use_ha)

plot(reg$production_kcal/reg$cropland_ha, reg$p_ha) #values are within 0.137 tons P/ha maximum (from World Bank) ; 5698 values are NA
boxplot(reg$p_ha) #visual representation of data points and outliers

plot(reg$production_kcal/reg$cropland_ha, reg$fuel_ha) #there are some values for which yields are 0 but fuel emissions per ha is above 0.010 Gg/ha, values that are higher than 0.015 Gg/ha are all from Singapore, everything else is lower than 0.010
boxplot(reg$fuel_ha) #429 outliers

plot(reg$production_kcal/reg$cropland_ha, reg$syn_n_ha) #some high values have nearly 0 yields,  2012 values are NA, values are lower in magnitude 
boxplot(reg$syn_n_ha)
ggplot(reg) + geom_density(aes(syn_n_ha))

clean_reg <- reg
write_csv(clean_reg, path  ="~/Google Drive/My Drive/Food & Farming/Energy in ag/Energy and GHG Analysis (2019)/Analysis/energy_ag_analysis/clean_data/clean_reg.csv")
...
#Member Countres within each region are from UN-NEPAD Africa Ag report and UN Statistics Classification: https://www.un.org/en/africa/osaa/pdf/pubs/2013africanagricultures.pdf

 W_Afr <- c("Benin","Burkina Faso","Côte d'Ivoire", "Cabo Verde", "Ghana", "Guinea", "Liberia", "Mali", "Niger", "Nigeria", "Senegal" ,"Sierra Leone",   "Gambia", "Togo")
#Guinea-Bissau, Saint Helena, Western Sahara in FOFA. What FOFA calls Cape Verde, is Cabo #Verde in FAOSTAT. 
 E_Afr <- c("Ethiopia", "Ethiopia PDR", "Kenya" ,  "Madagascar" , "Mauritius","Rwanda", "Somalia", "United Republic of Tanzania", "Uganda")
#Djibouti, South Sudan, Eritrea, Comoros, Reunion, Réunion, Mayotte, Seychelles not in FOFA
 C_Afr <- c("Burundi","Chad",  "Central African Republic", "Gabon","Democratic Republic of the Congo", "Cameroon")
 ...
 S_Afr <-c("Angola","Botswana", "Eswatini", "Lesotho", "Namibia", "South Africa", "Mozambique", "Malawi", "Zambia", "Zimbabwe" ) #FOFA uses old name for Eswatini which is Swaziland, but FAOSTAT uses Eswatini 
 N_Afr <-c("Algeria", "Egypt", "Morocco", "Tunisia","Mauritania")
#No Western Sahara or Sudan(former) in FOFA, no Libya

#create new coumn with names as factors to revise
clean_reg <- mutate(clean_reg, Country_rename = country)

# # recode African country names 
 clean_reg$Country_rename[clean_reg$Country_rename %in% E_Afr] <-  "E_Afr"
 clean_reg$Country_rename[clean_reg$Country_rename %in% W_Afr] <-  "W_Afr"
 clean_reg$Country_rename[clean_reg$Country_rename %in% C_Afr] <-  "C_Afr"
 clean_reg$Country_rename[clean_reg$Country_rename %in% S_Afr] <-  "S_Afr"
 clean_reg$Country_rename[clean_reg$Country_rename %in% N_Afr] <-  "N_Afr"
 
# #filter data by region
 clean_reg_E <- dplyr::filter(clean_reg,clean_reg$Country_rename == "E_Afr" )
 clean_reg_W <- dplyr::filter(clean_reg,clean_reg$Country_rename == "W_Afr" )
 clean_reg_C <- dplyr::filter(clean_reg,clean_reg$Country_rename == "C_Afr" )
 clean_reg_S <- dplyr::filter(clean_reg,clean_reg$Country_rename == "S_Afr" )
 clean_reg_N <- dplyr::filter(clean_reg,clean_reg$Country_rename == "N_Afr" )
```

```{r Not necessary to run - amount of missing data}

 #Amount of missing data for E.Afica region
# nrow(clean_reg_E[clean_reg_E$syn_n_ha == "NA",])/nrow(clean_reg_E) * 100 #1.5% are NAs
# nrow(clean_reg_E[clean_reg_E$syn_n_use_ha == "NA",])/nrow(clean_reg_E) * 100 #1.5% are NAs
# nrow(clean_reg_E[clean_reg_E$p_ha == "NA",])/nrow(clean_reg_E) * 100 #54% of data are NAs
# nrow(clean_reg_E[clean_reg_E$fuel_ha == "NA",])/nrow(clean_reg_E) * 100 #67% are NAs
# 
# #Amount of missing data for W.Afica region
# nrow(clean_reg_W[clean_reg_W$syn_n_ha == "NA",])/nrow(clean_reg_W) * 100 #4% are NAs
# nrow(clean_reg_W[clean_reg_W$syn_n_use_ha == "NA",])/nrow(clean_reg_W) * 100 #4% are NAs
# nrow(clean_reg_W[clean_reg_W$p_ha == "NA",])/nrow(clean_reg_W) * 100 #65% of data are NAs
# nrow(clean_reg_W[clean_reg_W$fuel_ha == "NA",])/nrow(clean_reg_W) * 100 #75% are NAs
# 
# #Amount of missing data for C.Afica region
# nrow(clean_reg_C[clean_reg_C$syn_n_ha == "NA",])/nrow(clean_reg_C) * 100 #6% are NAs
# nrow(clean_reg_C[clean_reg_C$syn_n_use_ha == "NA",])/nrow(clean_reg_C) * 100 #6% are NAs
# nrow(clean_reg_C[clean_reg_C$p_ha == "NA",])/nrow(clean_reg_C) * 100 #65% of data are NAs
# nrow(clean_reg_C[clean_reg_C$fuel_ha == "NA",])/nrow(clean_reg_C) * 100 #86% data missing
# 
# #Amount of missing data for S.Afica region
# nrow(clean_reg_S[clean_reg_S$syn_n_ha == "NA",])/nrow(clean_reg_S) * 100 #8% are NAs
# nrow(clean_reg_S[clean_reg_S$syn_n_use_ha == "NA",])/nrow(clean_reg_S) * 100 #8% are NAs
# nrow(clean_reg_S[clean_reg_S$p_ha == "NA",])/nrow(clean_reg_S) * 100 #54% of data are NAs
# nrow(clean_reg_S[clean_reg_S$fuel_ha == "NA",])/nrow(clean_reg_S) * 100  #62%
# 
# #Amount of missing data for N.Afica region
# nrow(clean_reg_N[clean_reg_N$syn_n_ha == "NA",])/nrow(clean_reg_N) * 100 #5% are NAs
# nrow(clean_reg_N[clean_reg_N$syn_n_use_ha == "NA",])/nrow(clean_reg_N) * 100 #5% are NAs
# nrow(clean_reg_N[clean_reg_N$p_ha == "NA",])/nrow(clean_reg_N) * 100 #54% of data are NAs
# nrow(clean_reg_N[clean_reg_N$fuel_ha == "NA",])/nrow(clean_reg_N) * 100  #35%
```

```{r Aggregate data for each region}

clean_reg_E_aggregate <- clean_reg_E %>%
  dplyr::select(year,cropland_ha,production_kcal, energy_used_in_fishery,pesticides_total, synthetic_nitrogen_fertilizers, total_energy, transport_fuel_used_in_agriculture_excl_fishery, fuel_emissions, syn_n_use) %>% 
  dplyr::group_by(year) %>%
  dplyr::summarise_all(sum, na.rm= TRUE)

clean_reg_W_aggregate <- clean_reg_W %>%
  dplyr::select(year,cropland_ha,production_kcal, energy_used_in_fishery,pesticides_total, synthetic_nitrogen_fertilizers, total_energy, transport_fuel_used_in_agriculture_excl_fishery, fuel_emissions, syn_n_use) %>% 
  dplyr::group_by(year) %>%
  dplyr::summarise_all(sum, na.rm= TRUE)

clean_reg_C_aggregate <- clean_reg_C %>%
  dplyr::select(year,cropland_ha,production_kcal, energy_used_in_fishery,pesticides_total, synthetic_nitrogen_fertilizers, total_energy, transport_fuel_used_in_agriculture_excl_fishery, fuel_emissions, syn_n_use) %>% 
  dplyr::group_by(year) %>%
  dplyr::summarise_all(sum, na.rm= TRUE)

clean_reg_S_aggregate <- clean_reg_S %>%
  dplyr::select(year,cropland_ha,production_kcal, energy_used_in_fishery,pesticides_total, synthetic_nitrogen_fertilizers, total_energy, transport_fuel_used_in_agriculture_excl_fishery, fuel_emissions, syn_n_use) %>% 
  dplyr::group_by(year) %>%
  dplyr::summarise_all(sum, na.rm= TRUE)

clean_reg_N_aggregate <- clean_reg_N %>%
  dplyr::select(year,cropland_ha,production_kcal, energy_used_in_fishery,pesticides_total, synthetic_nitrogen_fertilizers, total_energy, transport_fuel_used_in_agriculture_excl_fishery, fuel_emissions, syn_n_use) %>% 
  dplyr::group_by(year) %>%
  dplyr::summarise_all(sum, na.rm= TRUE)

#West Africa
clean_reg_W_aggregate$yield <- clean_reg_W_aggregate$production_kcal/clean_reg_W_aggregate$cropland_ha
clean_reg_W_aggregate$fuel_ha <- clean_reg_W_aggregate$fuel_emissions/clean_reg_W_aggregate$cropland_ha
clean_reg_W_aggregate$p_ha <- clean_reg_W_aggregate$pesticides_total/clean_reg_W_aggregate$cropland_ha
clean_reg_W_aggregate$syn_n_ha <- clean_reg_W_aggregate$synthetic_nitrogen_fertilizers/clean_reg_W_aggregate$cropland_ha
clean_reg_W_aggregate$syn_n_use_ha <- clean_reg_W_aggregate$syn_n_use/clean_reg_W_aggregate$cropland_ha

clean_reg_W_aggregate$country <- "West" #recode countries so data can be aggregated by region

#Save West Africa only data to clean data folder
#write_csv(clean_reg_W_aggregate, path  ="~/Google Drive/My Drive/Food & Farming/Energy in ag/Energy and GHG Analysis (2019)/Analysis/energy_ag_analysis/clean_data/clean_reg_W_aggregate.csv")

#East Africa
clean_reg_E_aggregate$yield <- clean_reg_E_aggregate$production_kcal/clean_reg_E_aggregate$cropland_ha
clean_reg_E_aggregate$fuel_ha <- clean_reg_E_aggregate$fuel_emissions/clean_reg_E_aggregate$cropland_ha
clean_reg_E_aggregate$p_ha <- clean_reg_E_aggregate$pesticides_total/clean_reg_E_aggregate$cropland_ha
clean_reg_E_aggregate$syn_n_ha <- clean_reg_E_aggregate$synthetic_nitrogen_fertilizers/clean_reg_E_aggregate$cropland_ha
clean_reg_E_aggregate$syn_n_use_ha <- clean_reg_E_aggregate$syn_n_use/clean_reg_E_aggregate$cropland_ha

clean_reg_E_aggregate$country <- "East" #recode countries so data can be aggregated by region

#Save East Africa only data to clean data folder
#write_csv(clean_reg_E_aggregate, path  ="~/Google Drive//My Drive/Food & Farming/Energy in ag/Energy and GHG Analysis (2019)/Analysis/energy_ag_analysis/clean_data/clean_reg_e_aggregate.csv")

#Save Central Africa only data to clean data folder
clean_reg_C_aggregate$yield <- clean_reg_C_aggregate$production_kcal/clean_reg_C_aggregate$cropland_ha
clean_reg_C_aggregate$fuel_ha <- clean_reg_C_aggregate$fuel_emissions/clean_reg_C_aggregate$cropland_ha
clean_reg_C_aggregate$p_ha <- clean_reg_C_aggregate$pesticides_total/clean_reg_C_aggregate$cropland_ha
clean_reg_C_aggregate$syn_n_ha <- clean_reg_C_aggregate$synthetic_nitrogen_fertilizers/clean_reg_C_aggregate$cropland_ha
clean_reg_C_aggregate$syn_n_use_ha <- clean_reg_C_aggregate$syn_n_use/clean_reg_C_aggregate$cropland_ha

clean_reg_C_aggregate$country <- "Central"#recode countries so data can be aggregated by region

#Save Central Africa only data to clean data folder
#write_csv(clean_reg_C_aggregate, path  ="~/Google Drive/My Drive/Food & Farming/Energy in ag/Energy and GHG Analysis (2019)/Analysis/energy_ag_analysis/clean_data/clean_reg_C_aggregate.csv")

#South Africa
clean_reg_S_aggregate$yield <- clean_reg_S_aggregate$production_kcal/clean_reg_S_aggregate$cropland_ha
clean_reg_S_aggregate$fuel_ha <- clean_reg_S_aggregate$fuel_emissions/clean_reg_S_aggregate$cropland_ha
clean_reg_S_aggregate$p_ha <- clean_reg_S_aggregate$pesticides_total/clean_reg_S_aggregate$cropland_ha
clean_reg_S_aggregate$syn_n_ha <- clean_reg_S_aggregate$synthetic_nitrogen_fertilizers/clean_reg_S_aggregate$cropland_ha
clean_reg_S_aggregate$syn_n_use_ha <- clean_reg_S_aggregate$syn_n_use/clean_reg_S_aggregate$cropland_ha

clean_reg_S_aggregate$country <- "South"  #recode countries so data can be aggregated by region

#Save S Africa only data to clean data foldeer
#write_csv(clean_reg_S_aggregate, path  ="~/Google Drive/My Drive/Food & Farming/Energy in ag/Energy and GHG Analysis (2019)/Analysis/energy_ag_analysis/clean_data/clean_reg_S_aggregate.csv")

#North Africa
clean_reg_N_aggregate$yield <- clean_reg_N_aggregate$production_kcal/clean_reg_N_aggregate$cropland_ha
clean_reg_N_aggregate$fuel_ha <- clean_reg_N_aggregate$fuel_emissions/clean_reg_N_aggregate$cropland_ha
clean_reg_N_aggregate$p_ha <- clean_reg_N_aggregate$pesticides_total/clean_reg_N_aggregate$cropland_ha
clean_reg_N_aggregate$syn_n_ha <- clean_reg_N_aggregate$synthetic_nitrogen_fertilizers/clean_reg_N_aggregate$cropland_ha
clean_reg_N_aggregate$syn_n_use_ha <- clean_reg_N_aggregate$syn_n_use/clean_reg_N_aggregate$cropland_ha

clean_reg_N_aggregate$country <- "North"  #recode countries so data can be aggregated by region

#Save North Africa only data to clean data folder
#write_csv(clean_reg_N_aggregate, path  ="~/Google Drive/My Drive/Food & Farming/Energy in ag/Energy and GHG Analysis (2019)/Analysis/energy_ag_analysis/clean_data/clean_reg_N_aggregate.csv")
...
reg_other_country <- clean_reg

#Filter out Africa data, keep non-Africa data 
reg_other_country <- mutate(reg, Country_rename = country)
reg_other_country$Country_rename[reg_other_country$Country_rename %in% N_Afr] <-  "North" # recode 
reg_other_country <- dplyr::filter(reg_other_country,reg_other_country$Country_rename != "North" ) #filter so all countries' data except those in North Africa are included

reg_other_country$Country_rename[reg_other_country$Country_rename %in% S_Afr] <-  "South" # recode 
reg_other_country <- dplyr::filter(reg_other_country,reg_other_country$Country_rename != "South" ) #filter so only all countries' data except those in South Africa are included

reg_other_country$Country_rename[reg_other_country$Country_rename %in% W_Afr] <-  "West" # recode 
reg_other_country <- dplyr::filter(reg_other_country,reg_other_country$Country_rename != "West" ) #filter so only all countries' data except those in West Africa are included

reg_other_country$Country_rename[reg_other_country$Country_rename %in% E_Afr] <-  "East" # recode 
reg_other_country <- dplyr::filter(reg_other_country,reg_other_country$Country_rename != "East" ) #filter so only all countries' data except those in West Africa are included

reg_other_country$Country_rename[reg_other_country$Country_rename %in% C_Afr] <-  "Central" # recode 
reg_other_country <- dplyr::filter(reg_other_country,reg_other_country$Country_rename != "Central" ) #filter so only all countries' data except those in West Africa are included

#Eliminate African countries data that are not in FOFA data & not covered by regions above. These countries have data in FAOSTAT. 
reg_other_country <- dplyr::filter(reg_other_country,reg_other_country$Country_rename != "Sudan" )
reg_other_country <- dplyr::filter(reg_other_country,reg_other_country$Country_rename != "Sudan(former)" )
reg_other_country <- dplyr::filter(reg_other_country,reg_other_country$Country_rename != "Congo" )
reg_other_country <- dplyr::filter(reg_other_country,reg_other_country$Country_rename != "Djibouti")
reg_other_country <- dplyr::filter(reg_other_country,reg_other_country$Country_rename != "Equatorial Guinea" )
reg_other_country <- dplyr::filter(reg_other_country,reg_other_country$Country_rename != "Eritrea")
reg_other_country <- dplyr::filter(reg_other_country,reg_other_country$Country_rename != "Comoros")
reg_other_country <- dplyr::filter(reg_other_country,reg_other_country$Country_rename != "Seychelles")
reg_other_country <- dplyr::filter(reg_other_country,reg_other_country$Country_rename != "Sao Tome and Principe")
reg_other_country <- dplyr::filter(reg_other_country,reg_other_country$Country_rename != "Guinea-Bissau")
reg_other_country <- dplyr::filter(reg_other_country,reg_other_country$Country_rename != "Réunion")
reg_other_country <- dplyr::filter(reg_other_country,reg_other_country$Country_rename != "Libya")

#Calculate Yields for all the other countries 
reg_other_country$yield <- reg_other_country$production_kcal/reg_other_country$cropland_ha

#Join Africa data with all of the countries' (non Africa) data
reg_other_country <- reg_other_country %>%
  dplyr::select(country,year, energy_used_in_fishery, synthetic_nitrogen_fertilizers,syn_n_use,  total_energy, transport_fuel_used_in_agriculture_excl_fishery,pesticides_total, fuel_emissions,fuel_ha, p_ha, syn_n_ha,syn_n_use_ha, yield,production_kcal, cropland_ha)

reg_all_countries <- dplyr::full_join(reg_other_country, clean_reg_W_aggregate) 

reg_all_countries <- dplyr::full_join(reg_all_countries, clean_reg_E_aggregate)  

reg_all_countries <- dplyr::full_join(reg_all_countries, clean_reg_S_aggregate)

reg_all_countries <- dplyr::full_join(reg_all_countries , clean_reg_C_aggregate)

reg_all_countries <- dplyr::full_join(reg_all_countries , clean_reg_N_aggregate) 
...
summary(reg_all_countries$syn_n_use)
boxplot(reg_all_countries$syn_n_use)
hist(reg_all_countries$syn_n_use,
  xlab = "syn_n_use",
  main = "Histogram of fuel",
  breaks = sqrt(nrow(reg_all_countries)))
...
#world_reg$fuel_ha <- world_reg$fuel_emissions/world_reg$cropland_ha
#world_reg$p_ha <- world_reg$pesticides_total/world_reg$cropland_ha
#world_reg$syn_n_ha <- world_reg$synthetic_nitrogen_fertilizers/world_reg$cropland_ha
#world_reg$syn_n_use_ha <- world_reg$syn_n_use/world_reg$cropland_ha
...
# filter_reg$syn_n_use <- filter_reg$syn_n_use * 3
# filter_reg$syn_n_ha <- filter_reg$synthetic_nitrogen_fertilizers/filter_reg$cropland_ha
# filter_reg$syn_n_use_ha <- filter_reg$syn_n_use/filter_reg$cropland_ha
...
filter_f_reg <- filter(filter_reg, filter_reg$fuel_ha > 0)
fuel.ols <- lm(filter_f_reg , formula = log(fuel_ha) ~ log(yield) + factor(country))
#filter_f_reg_2 <- filter(filter_reg_2, filter_reg_2$fuel_ha > 0)
#fuel.ols <- lm(filter_f_reg_2 , formula = log(fuel_ha) ~ log(yield) + factor(country)) #Global yield sensitivity 
#fuel.ols <- lm(filter_f_reg_2 , formula = log(fuel_ha) ~ log(yield) + factor(country))

#filter_f_reg_yrs <- filter(filter_reg_yrs, filter_reg_yrs$fuel_ha > 0)
#fuel.ols <- lm(filter_f_reg_yrs , formula = log(fuel_ha) ~ log(yield) + factor(country)) #Global yield sensitivity 


#Synthetic N emissions OLS Regression 
filter_syn_reg <- filter(filter_reg, filter_reg$syn_n_ha > 0)
syn_n.ols <- lm(filter_syn_reg, formula = log(syn_n_ha) ~ log(yield) + factor(country)) #no #outliers in boxplot

#filter_syn_reg_yrs <- filter(filter_reg_yrs, filter_reg_yrs$syn_n_ha > 0)
#syn_n.ols <- lm(filter_syn_reg_yrs, formula = log(syn_n_ha) ~ log(yield) + factor(country))

#Global yield sensitivity 
#filter_syn_reg_2 <- filter(filter_reg_2, filter_reg_2$syn_n_ha > 0)
#syn_n.ols <- lm(filter_syn_reg_2, formula = log(syn_n_ha) ~ log(yield) + factor(country))

#Synthetic N use or application OLS regression
filter_syn_n_use_reg <- filter(filter_reg, filter_reg$syn_n_use_ha > 0)
syn_n_use.ols <- lm(filter_syn_n_use_reg, formula = log(syn_n_use_ha) ~ log(yield) + factor(country))

#filter_syn_n_use_reg_yrs <- filter(filter_reg_yrs, filter_reg_yrs$syn_n_use_ha > 0)
#syn_n_use.ols <- lm(filter_syn_n_use_reg_yrs, formula = log(syn_n_use_ha) ~ log(yield) + factor(country))
#Global yield sensitivity 
#filter_syn_n_use_reg_2 <- filter(filter_reg_2, filter_reg_2$syn_n_use_ha > 0)
#syn_n_use.ols <- lm(filter_syn_n_use_reg_2, formula = log(syn_n_use_ha) ~ log(yield) + factor(country))

```

```{r Import yields for each scenario and region to estumate input use and emissions}

regions_scenario <- read_csv("~/Google Drive/My Drive/Food & Farming/Energy in ag/LandDemand/regions_scenario_dan2.csv") #yields are in kcal/ha
regions_scenario <- regions_scenario_dan2
#Convert country names in regions_scenario to East, West, Central, South, North
regions_scenario$region <- recode(regions_scenario$region, E_Afr = "East", C_Afr = "Central", S_Afr = "South", N_Afr = "North", W_Afr = "West")
#regions_scenario$...1 <- NULL

b <- filter(regions_scenario, scenario == "b")
...
p <- filter(regions_scenario, scenario == "p")
...
h_alt <- filter(regions_scenario, scenario == "h")
...
# 
# synNuse_b <- exp(predict(syn_n_use.ols, newdata  = b, interval = "confidence", level = 0.95))
# synNuse_p <- exp(predict(syn_n_use.ols, newdata  = p, interval = "confidence", level = 0.95))
# synNuse_h <- exp(predict(syn_n_use.ols, newdata  = h_alt, interval = "confidence", level = 0.95))
# 
...
fuel_ha_E <- rbind(fuel_b_e, fuel_proj_e ,  fuel_h_alt_e)
colnames(fuel_ha_E) <- c("fit_fuel_ha_e","lwr_fuel_ha_e", "upr_fuel_ha_e")
...
fuel_ha_W <- rbind(fuel_b_w, fuel_proj_w , fuel_h_alt_w)
colnames(fuel_ha_W) <- c("fit_fuel_ha_w","lwr_fuel_ha_w", "upr_fuel_ha_w")
...
fuel_ha_S <- rbind(fuel_b_s  , fuel_proj_s ,  fuel_h_alt_s)
colnames(fuel_ha_S) <- c("fit - fuel_ha_s","lwr - fuel_ha_s", "upr)fuel_ha_s")
...
fuel_ha_C <- rbind(fuel_b_c , fuel_proj_c ,  fuel_h_alt_c)
colnames(fuel_ha_C) <- c("fit_fuel_ha_c","lwr_fuel_ha_c", "upr_fuel_ha_c")
...
fuel_ha_N <- rbind(fuel_b_n  , fuel_proj_n  , fuel_h_alt_n)
colnames(fuel_ha_N) <- c("fit_fuel_ha_n","lwr_fuel_ha_n", "upr_fuel_ha_n")

 #Save fuel emissions per ha as table for working paper
#fuel_ha_W <- as.data.frame(fuel_ha_W) 
#fuel_ha_W$Region <- "West Africa"
# fuel_ha_E <- as.data.frame(fuel_ha_E) 
# fuel_ha_E$Region <- "East Africa"
# fuel_ha_N <- as.data.frame(fuel_ha_N) 
#fuel_ha_N$Region <- "North Africa"
#fuel_ha_S <- as.data.frame(fuel_ha_S) 
# fuel_ha_S$Region <- "South Africa"
# fuel_ha_C <- as.data.frame(fuel_ha_C) 
# fuel_ha_C$Region <- "Central Africa"
# 
# fuel_ha_W$Scenario <- c("Baseline", "Projected","High")
# fuel_ha_S$Scenario <- c("Baseline", "Projected","High")
# fuel_ha_C$Scenario <- c("Baseline", "Projected","High")
#fuel_ha_N$Scenario <- c("Baseline", "Projected","High")
 #fuel_ha_E$Scenario <- c("Baseline", "Projected","High")
# 
 #colnames(fuel_ha_W) <- c("fit", "lwr", "upr", "Region", "Scenario")
 #colnames(fuel_ha_E) <- c("fit", "lwr", "upr", "Region", "Scenario")
# colnames(fuel_ha_S) <- c("fit", "lwr", "upr", "Region", "Scenario")
#colnames(fuel_ha_N) <- c("fit", "lwr", "upr", "Region", "Scenario")
#colnames(fuel_ha_C) <- c("fit", "lwr", "upr", "Region", "Scenario")
# 
 #fuel_ha_tbl <- rbind(fuel_ha_W,fuel_ha_S, fuel_ha_C, fuel_ha_N, fuel_ha_E )
#write.csv(fuel_ha_tbl,"~/Google Drive/My Drive/Food & Farming/Energy in ag/results/Energy-Ag_tbls/fuel_ha.csv")
...
synN_ha_E <- rbind(synN_b_e, synN_proj_e ,  synN_h_alt_e)
colnames(synN_ha_E) <- c("fit_synN_ha_e","lwr_synN_ha_e", "upr_synN_ha_e")
...
synN_ha_W <- rbind(synN_b_w, synN_proj_w ,  synN_h_alt_w)
colnames(synN_ha_W) <- c("fit_synN_ha_w","lwr_synN_ha_w", "upr_synN_ha_w")
...
synN_ha_S <- rbind(synN_b_s , synN_proj_s ,  synN_h_alt_s)
colnames(synN_ha_S) <- c("fit_synN_ha_s","lwr_synN_ha_s", "upr_synN_ha_s")
...
synN_ha_C <- rbind(synN_b_c , synN_proj_c , synN_h_alt_c)
colnames(synN_ha_C) <- c("fit_synN_ha_c","lwr_synN_ha_c", "upr_synN_ha_c")
...
synN_ha_N <- rbind(synN_b_n , synN_proj_n ,  synN_h_alt_n)
colnames(synN_ha_N) <- c("fit_synN_ha_n","lwr_synN_ha_n", "upr_synN_ha_n")
...
synNuse_b_e <- exp(predict(syn_n_use.ols, newdata  = b_e, interval = "confidence", level = 0.95))
synNuse_proj_e <- exp(predict(syn_n_use.ols, newdata  = p_e,  interval = "confidence", level = 0.95))
synNuse_h_alt_e <- exp(predict(syn_n_use.ols, newdata  = h_alt_e,  interval = "confidence", level = 0.95))

# synNuse_b_e <- predict(syn_n_use.ols, newdata  = b_e, interval = "confidence", level = 0.95)
# synNuse_proj_e <- predict(syn_n_use.ols, newdata  = p_e,  interval = "confidence", level = 0.95)
# synNuse_h_alt_e <- predict(syn_n_use.ols, newdata  = h_alt_e,  interval = "confidence", level = 0.95)
...
#Make predictions on synthetic N use for each scenario for West Africa
 synNuse_b_w <- exp(predict(syn_n_use.ols, newdata  = b_w, interval = "confidence", level = 0.95))
 synNuse_proj_w <- exp(predict(syn_n_use.ols, newdata  = p_w,  interval = "confidence", level = 0.95))
 synNuse_h_alt_w <- exp(predict(syn_n_use.ols, newdata  = h_alt_w,  interval = "confidence", level = 0.95))

# synNuse_b_w <- predict(syn_n_use.ols, newdata  = b_w, interval = "confidence", level = 0.95)
# synNuse_proj_w <- predict(syn_n_use.ols, newdata  = p_w,  interval = "confidence", level = 0.95)
# synNuse_h_alt_w <- predict(syn_n_use.ols, newdata  = h_alt_w,  interval = "confidence", level = 0.95)
...
synNuse_b_s <- exp(predict(syn_n_use.ols, newdata  = b_s, interval = "confidence", level = 0.95))
synNuse_proj_s <- exp(predict(syn_n_use.ols, newdata  = p_s,  interval = "confidence", level = 0.95))
synNuse_h_alt_s <- exp(predict(syn_n_use.ols, newdata  = h_alt_s,  interval = "confidence", level = 0.95))

#for senstiviity analysis, linear model
# synNuse_b_s <- predict(syn_n_use.ols, newdata  = b_s, interval = "confidence", level = 0.95)
# synNuse_proj_s <- predict(syn_n_use.ols, newdata  = p_s,  interval = "confidence", level = 0.95)
# synNuse_h_alt_s <- predict(syn_n_use.ols, newdata  = h_alt_s,  interval = "confidence", level = 0.95)
...
#Make predictions on Synthetic N use use for each scenario for Central Africa
 synNuse_b_c <- exp(predict(syn_n_use.ols, newdata  = b_c, interval = "confidence", level = 0.95))
 synNuse_proj_c <- exp(predict(syn_n_use.ols, newdata  = p_c,  interval = "confidence", level = 0.95))
 synNuse_h_alt_c <- exp(predict(syn_n_use.ols, newdata  = h_alt_c,  interval = "confidence", level = 0.95))

#for senstiviity analysis, linear model
# synNuse_b_c <- predict(syn_n_use.ols, newdata  = b_c, interval = "confidence", level = 0.95)
# synNuse_proj_c <- predict(syn_n_use.ols, newdata  = p_c,  interval = "confidence", level = 0.95)
# synNuse_h_alt_c <- predict(syn_n_use.ols, newdata  = h_alt_c,  interval = "confidence", level = 0.95)
...
#Make predictions on synthetic N use for each scenario for North Africa
 synNuse_b_n <- exp(predict(syn_n_use.ols, newdata = b_n, interval = "confidence", level = 0.95))
synNuse_proj_n <- exp(predict(syn_n_use.ols, newdata  = p_n,  interval = "confidence", level = 0.95))
 synNuse_h_alt_n <- exp(predict(syn_n_use.ols, newdata  = h_alt_n,  interval = "confidence", level = 0.95))

#for senstiviity analysis, linear model
# synNuse_b_n <- predict(syn_n_use.ols, newdata = b_n, interval = "confidence", level = 0.95)
# synNuse_proj_n <- predict(syn_n_use.ols, newdata  = p_n,  interval = "confidence", level = 0.95)
# synNuse_h_alt_n <- predict(syn_n_use.ols, newdata  = h_alt_n,  interval = "confidence", level = 0.95)
...
# predict_tbl_E <- as.data.frame(cbind(p_ha_E,fuel_ha_E,synN_ha_E, synNuse_ha_E))
# rownames(predict_tbl_E) <- c("Baseline", "Projected",  "High_alt")
# predict_tbl_W <- as.data.frame(cbind(p_ha_W,fuel_ha_W,synN_ha_W, synNuse_ha_W))
# rownames(predict_tbl_W) <- c("Baseline", "Projected",  "High_alt")
# predict_tbl_S <- as.data.frame(cbind(p_ha_S,fuel_ha_S,synN_ha_S, synNuse_ha_S))
# rownames(predict_tbl_S) <- c("Baseline", "Projected", "High_alt")
# predict_tbl_C <- as.data.frame(cbind(p_ha_C,fuel_ha_C,synN_ha_C, synNuse_ha_C))
# rownames(predict_tbl_C) <- c("Baseline", "Projected",  "High_alt")
# predict_tbl_N <- as.data.frame(cbind(p_ha_N,fuel_ha_N,synN_ha_N, synNuse_ha_N))
...
area_b <- regions_scenario %>% dplyr::select(harv_area, region, scenario) %>% filter(scenario == "b")
area_p <- regions_scenario %>% dplyr::select(harv_area, region, scenario) %>% filter( scenario == "p")
#area_h <- regions_scenario %>% dplyr::select(harv_area, region, scenario) %>% filter(scenario == "high")
area_h_alt <- regions_scenario %>% dplyr::select(harv_area, region, scenario) %>% filter( scenario == "h")
...
# 
# 
# plot(x = log(na.omit(filter_f_reg$fuel_ha)), y = fuel.ols$fitted.values, xlab = "True values", ylab = "Model Fitted values", main = "Regression fits of fuel/ha values")
#  abline(b=1, a=0) #values should fall around this line, some values are scattered to the left 
# 
# 
# plot(x = log(na.omit(filter_syn_reg$syn_n_ha)), y = syn_n.ols$fitted.values, xlab = "True values", ylab = "Model Fitted values", main = "Regression fits of synN/ha values")
# abline(b=1, a=0) #values fall around this 1:1 line 
# 
# plot(log(na.omit(filter_syn_n_use_reg$syn_n_use_ha)), syn_n_use.ols$fitted.values,xlab = "True values", ylab = "Model Fitted values", main = "Regression fits of synN use/ha values"  )
# abline(b=1, a=0) #values fall around this 1:1 line 
```
...
clean_reg_m <- full_join(clean_reg, machinery_emissions)
clean_reg_m <- clean_reg_m %>% dplyr::select(year,country,cropland_ha, production_kcal, machinery.emissions)
clean_reg_m$country_rename <- clean_reg_m$country


#Aggregate clean_reg_m data by region, rename the country names according to their region in Africa.
clean_reg_m$country_rename[clean_reg_m$country_rename %in% E_Afr] <-  "E_Afr"
clean_reg_m$country_rename[clean_reg_m$country_rename %in% W_Afr] <-  "W_Afr"
clean_reg_m$country_rename[clean_reg_m$country_rename %in% C_Afr] <-  "C_Afr"
clean_reg_m$country_rename[clean_reg_m$country_rename %in% S_Afr] <-  "S_Afr"
clean_reg_m$country_rename[clean_reg_m$country_rename %in% N_Afr] <-  "N_Afr"

clean_reg_m_E <- dplyr::filter(clean_reg_m,clean_reg_m$country_rename == "E_Afr" )
clean_reg_m_W <- dplyr::filter(clean_reg_m,clean_reg_m$country_rename == "W_Afr" )
clean_reg_m_C <- dplyr::filter(clean_reg_m,clean_reg_m$country_rename == "C_Afr" )
clean_reg_m_S <- dplyr::filter(clean_reg_m,clean_reg_m$country_rename == "S_Afr" )
clean_reg_m_N <- dplyr::filter(clean_reg_m,clean_reg_m$country_rename == "N_Afr" )
...
clean_reg_E_aggregate <- clean_reg_m_E %>%
  dplyr::select(year, cropland_ha,production_kcal, machinery.emissions) %>% 
  dplyr::group_by(year) %>%
  dplyr::summarise_all(sum, na.rm = TRUE)
clean_reg_W_aggregate <- clean_reg_m_W %>%
  dplyr::select(year, cropland_ha,production_kcal, machinery.emissions) %>% 
  dplyr::group_by(year) %>%
  dplyr::summarise_all(sum, na.rm = TRUE)
clean_reg_S_aggregate <- clean_reg_m_S %>%
  dplyr::select(year, cropland_ha,production_kcal,machinery.emissions) %>% 
  dplyr::group_by(year) %>%
  dplyr::summarise_all(sum, na.rm = TRUE)
clean_reg_N_aggregate <- clean_reg_m_N %>%
  dplyr::select(year, cropland_ha,production_kcal, machinery.emissions) %>% 
  dplyr::group_by(year) %>%
  dplyr::summarise_all(sum, na.rm = TRUE)
clean_reg_C_aggregate <- clean_reg_m_C %>%
  dplyr::select(year, cropland_ha,production_kcal, machinery.emissions) %>% 
  dplyr::group_by(year) %>%
  dplyr::summarise_all(sum, na.rm = TRUE)

clean_reg_E_aggregate$m_ha <- clean_reg_E_aggregate$machinery.emissions/clean_reg_E_aggregate$cropland_ha
clean_reg_W_aggregate$m_ha <- clean_reg_W_aggregate$machinery.emissions/clean_reg_W_aggregate$cropland_ha
clean_reg_S_aggregate$m_ha <- clean_reg_S_aggregate$machinery.emissions/clean_reg_S_aggregate$cropland_ha
clean_reg_C_aggregate$m_ha <- clean_reg_C_aggregate$machinery.emissions/clean_reg_C_aggregate$cropland_ha
clean_reg_N_aggregate$m_ha <- clean_reg_N_aggregate$machinery.emissions/clean_reg_N_aggregate$cropland_ha
...
clean_reg_E_aggregate$country <- "East"
clean_reg_W_aggregate$country <- "West"
clean_reg_S_aggregate$country <- "Southern"
clean_reg_C_aggregate$country<- "Central"
clean_reg_N_aggregate$country <- "North"
...
reg_other_country <- inner_join(reg_other_country, clean_reg_m) # add machinery values to non-Africa dataframe

reg_all_countries <- full_join(reg_other_country ,clean_reg_E_aggregate )
reg_all_countries <- full_join(reg_all_countries ,clean_reg_W_aggregate )
reg_all_countries <- full_join(reg_all_countries ,clean_reg_S_aggregate )
reg_all_countries <- full_join(reg_all_countries,clean_reg_C_aggregate )
reg_all_countries <- full_join(reg_all_countries ,clean_reg_N_aggregate )
...
predict_tbl_E <- as.data.frame(cbind(p_ha_E,fuel_ha_E,synN_ha_E, synNuse_ha_E, machinery_ha_E))
rownames(predict_tbl_E) <- c("Baseline", "Projected",  "High_alt")
predict_tbl_W <- as.data.frame(cbind(p_ha_W,fuel_ha_W,synN_ha_W, synNuse_ha_W, machinery_ha_W))
rownames(predict_tbl_W) <- c("Baseline", "Projected",  "High_alt")
predict_tbl_S <- as.data.frame(cbind(p_ha_S,fuel_ha_S,synN_ha_S, synNuse_ha_S, machinery_ha_S))
rownames(predict_tbl_S) <- c("Baseline", "Projected", "High_alt")
predict_tbl_C <- as.data.frame(cbind(p_ha_C,fuel_ha_C,synN_ha_C, synNuse_ha_C, machinery_ha_c))
rownames(predict_tbl_C) <- c("Baseline", "Projected",  "High_alt")
predict_tbl_N <- as.data.frame(cbind(p_ha_N,fuel_ha_N,synN_ha_N, synNuse_ha_N, machinery_ha_n))
rownames(predict_tbl_N) <- c("Baseline", "Projected",  "High_alt")
...
predict_tbl_C$Energy_Source <- str_replace(predict_tbl_C$Energy_Source, "fuel_ha_C","On-Farm Fuel")
predict_tbl_C$Energy_Source  <- str_replace(predict_tbl_C$Energy_Source , "synN_ha_C","Synthetic N")
...
predict_tbl_E$Energy_Source <- str_replace(predict_tbl_E$Energy_Source, "fuel_ha_e","On-Farm Fuel")
predict_tbl_E$Energy_Source  <- str_replace(predict_tbl_E$Energy_Source , "synN_ha_E","Synthetic N")
...
predict_tbl_W$Energy_Source <- str_replace(predict_tbl_W$Energy_Source, "fuel_ha_W","On-Farm Fuel")
predict_tbl_W$Energy_Source  <- str_replace(predict_tbl_W$Energy_Source , "synN_ha_W","Synthetic N")
...
predict_tbl_N$Energy_Source <- str_replace(predict_tbl_N$Energy_Source, "fuel_ha_N","On-Farm Fuel")
predict_tbl_N$Energy_Source  <- str_replace(predict_tbl_N$Energy_Source , "synN_ha_N","Synthetic N")
...
predict_tbl_S$Energy_Source <- str_replace(predict_tbl_S$Energy_Source, "fuel_ha_S","On-Farm Fuel")
predict_tbl_S$Energy_Source  <- str_replace(predict_tbl_S$Energy_Source , "synN_ha_S","Synthetic N")
...
#predict_tbl_C$mean_f <- rowMeans(cbind(predict_tbl_C$`lwr - fuel_ha_C`, predict_tbl_C$`upr - fuel_ha_C`)) 
#predict_tbl_C$mean_synNuse <- rowMeans(cbind(predict_tbl_C$`lwr - synNuse_ha_c`, predict_tbl_C$`upr - synNuse_ha_c`)) 
#predict_tbl_C$mean_synN <- rowMeans(cbind(predict_tbl_C$`lwr - synN_ha_C`, predict_tbl_C$`upr - synN_ha_C`)) 
...
# predict_tbl_N$mean_f <- rowMeans(cbind(predict_tbl_N$`lwr - fuel_ha_N`, predict_tbl_N$`upr - fuel_ha_N`)) 
# predict_tbl_N$mean_synNuse <- rowMeans(cbind(predict_tbl_N$`lwr - synNuse_ha_n`, predict_tbl_N$`upr - synNuse_ha_n`)) 
# predict_tbl_N$mean_synN <- rowMeans(cbind(predict_tbl_N$`lwr - synN_ha_N`, predict_tbl_N$`upr - synN_ha_N`)) 
...
# predict_tbl_W$mean_f <- rowMeans(cbind(predict_tbl_W$`lwr - fuel_ha_W`, predict_tbl_W$`upr - fuel_ha_W`)) 
# predict_tbl_W$mean_synNuse <- rowMeans(cbind(predict_tbl_W$`lwr - syn_Nuse_ha_w`, predict_tbl_W$`upr - syn_Nuse_ha_w`)) 
# predict_tbl_W$mean_synN <- rowMeans(cbind(predict_tbl_W$`lwr - synN_ha_W`, predict_tbl_W$`upr - synN_ha_W`)) 
...
# predict_tbl_S$mean_f <- rowMeans(cbind(predict_tbl_S$`lwr - fuel_ha_S`, predict_tbl_S$`upr - fuel_ha_S`)) 
# predict_tbl_S$mean_synNuse <- rowMeans(cbind(predict_tbl_S$`lwr - synNuse_ha_s`, predict_tbl_S$`upr - synNuse_ha_s`)) 
# predict_tbl_S$mean_synN <- rowMeans(cbind(predict_tbl_S$`lwr - synN_ha_S`, predict_tbl_S$`upr - synN_ha_S`)) 
...
# predict_tbl_E$mean_f <- rowMeans(cbind(predict_tbl_E$`lwr - fuel_ha_e`, predict_tbl_E$`upr - fuel_ha_e`)) 
# predict_tbl_E$mean_synNuse <- rowMeans(cbind(predict_tbl_E$`lwr - syn_Nuse_ha_e`, predict_tbl_E$`upr - syn_Nuse_ha_e`)) 
# predict_tbl_E$mean_synN <- rowMeans(cbind(predict_tbl_E$`lwr - synN_ha_E`, predict_tbl_E$`upr - synN_ha_E`)) 
...
# #predict_tbl$mean_f <- rowMeans(cbind(predict_tbl$`lwr - fuel_ha`, predict_tbl$`upr - fuel_ha`)) 
# #predict_tbl$mean_synNuse <- rowMeans(cbind(predict_tbl$`lwr - syn_n_use_ha`, predict_tbl$`upr - syn_n_use_ha`)) 
# #predict_tbl$mean_synN <- rowMeans(cbind(predict_tbl$`lwr - syn_n_ha`, predict_tbl$`upr - syn_n_ha`)) 
...
# 
# f_lwr_N <- ((predict_tbl_N$`lwr - fuel_ha_N` + predict_tbl_N$mean_f)/1.96)
# f_lwr_S <- ((predict_tbl_S$`lwr - fuel_ha_S` + predict_tbl_S$mean_f)/1.96)
# f_lwr_C <- ((predict_tbl_C$`lwr - fuel_ha_C` + predict_tbl_C$mean_f)/1.96)
# f_lwr_W <- ((predict_tbl_W$`lwr - fuel_ha_W` + predict_tbl_W$mean_f)/1.96) 
# f_lwr_E <- ((predict_tbl_E$`lwr - fuel_ha_e` + predict_tbl_E$mean_f)/1.96)

# f_upr_N <- ((predict_tbl_N$`upr - fuel_ha_N`  + predict_tbl_N$mean_f)/1.96)
# f_upr_S <- ((predict_tbl_S$`upr - fuel_ha_S` + predict_tbl_S$mean_f)/1.96)
# f_upr_C <- ((predict_tbl_C$`upr - fuel_ha_C` + predict_tbl_C$mean_f)/1.96)
# f_upr_W <- ((predict_tbl_W$`upr - fuel_ha_W` + predict_tbl_W$mean_f)/1.96)
# f_upr_E <- ((predict_tbl_E$`upr - fuel_ha_e` + predict_tbl_E$mean_f)/1.96)
# 
...
# 
# synN_lwr_N <- ((predict_tbl_N$`lwr - synN_ha_N` + predict_tbl_N$mean_synN)/1.96)
# synN_lwr_S <- ((predict_tbl_S$`lwr - synN_ha_S` + predict_tbl_S$mean_synN)/1.96)
# synN_lwr_C <- ((predict_tbl_C$`lwr - synN_ha_C` + predict_tbl_C$mean_synN)/1.96)
# synN_lwr_W <- ((predict_tbl_W$`lwr - synN_ha_W` + predict_tbl_W$mean_synN)/1.96)
# synN_lwr_E <- ((predict_tbl_E$`lwr - synN_ha_E` + predict_tbl_E$mean_synN)/1.96) 
# 
# synN_upr_N <- ((predict_tbl_N$`upr - synN_ha_N` + predict_tbl_N$mean_synN)/1.96)
# synN_upr_S <- ((predict_tbl_S$`upr - synN_ha_S` + predict_tbl_S$mean_synN)/1.96)
# synN_upr_C <- ((predict_tbl_C$`upr - synN_ha_C` + predict_tbl_C$mean_synN)/1.96)
# synN_upr_W <- ((predict_tbl_W$`upr - synN_ha_W` + predict_tbl_W$mean_synN)/1.96)
# synN_upr_E <- ((predict_tbl_E$`upr - synN_ha_E` + predict_tbl_E$mean_synN)/1.96)
# 
...
syn_n_use.coef  <- rbind(syn_n_use.ols$coefficients[1],syn_n_use.ols$coefficients[2])
...
energy_coefs <- cbind(m.ols_coef, p.ols_coef, syn_n.coef, syn_n_use.coef, fuel.coef)
...
syn_n_use.coeffs <- as.data.frame(syn_n_use.ols$coefficients)
syn_n_use.coeffs$coeffs <- rownames(syn_n_use.coeffs)
rownames(syn_n_use.coeffs) <- NULL
write.csv(syn_n_use.coeffs, "~/Desktop/Energy-Ag_tbls/syn_n_use_coeffs.csv")
