...
scenario_yields <- select(scenarios, country=region, yield, scenario)
...
reg_fe <- function(dependent_variable, scenario, region){
  output <- exp(predict(dependent_variable, 
              newdata = scenario_yields[scenario_yields$country==region & scenario_yields$scenario==scenario, c("country", "yield")], 
...
regions <- unique(scenarios$region)
regions <- regions[!regions =="Africa"]

#map variables to model names
var_models = tibble(var = c("en_emiss_ha", "fert_use_ha", "fert_emiss_ha", "p_ha"),
                   model = c("fuel.ols", "fert_use.ols", "syn_n.ols", "p.ols"))
...
reg_fe_results <- reg_fe_results %>% 
  mutate(dep_var = recode(dep_var, #change var names for resulting table. for recode, format is old = new
                          "fert_use_ha"="f_m", "p_ha"="pest", "en_emiss_ha"="en", "fert_emiss_ha"="f_a")) %>% 
  left_join(scenarios) %>% #add production scenario variables i.e. yield, area to fe regression results table. join leaves out total Africa region
...
```{r Old Calculate emissions for each input source, region, and scenario}
