...
p_ols <- lm(filter_reg, formula = log(pest_use_ha) ~ log(yield) + factor(country))
...
var_models = tibble(var = c("en_emiss_ha", "fert_emiss_ha", "fert_m_emiss_ha", "m_emiss_ha", "pest_emiss_ha", "tot_emiss_ha", "pest_use_ha", "fert_use_ha"),
...
convert_luc_co2 <- function(x){return(x * kg_to_mt/ (2050-2025))} # convert to million metric ton and divide by number of years between 2025 and 2050, this division annualizes the emissions
...
# Load vectors of countries in regions
load("regions.RData")
...
reg_fe_results %>% 
  mutate(region = recode(region, "Middle" = "Middle Africa", "Eastern" = "Eastern Africa", "Northern" = "Northern Africa", "Southern" = "Southern Africa", "Western" = "Western Africa")) %>%  #change to full names for regions
  filter(dep_var=="pest_use_ha", region!="Africa") %>% #filter to just pesticide fertilizer application rate, and exclude Africa total
  select(-lwr, -upr) %>% #remove upper and lower bounds
  pivot_wider(names_from = dep_var, values_from = fit) %>% #pivot to have 1 column per source
  mutate(pest_use_ha=pest_use_ha*1000) %>%  #convert tons/ha to kg/ha
  rename(Scenario = scenario, Region = region, `Pesticide Application (kg active ingredient/ha)` = pest_use_ha) %>%  #rename columns for presentation
  arrange(Scenario, Region) %>% #order by scenario order, and regions alphabetically within
  mutate(across(-c(Scenario, Region), ~round(., digits = decimals))) %>% #round values
...
africa_linear_sums %>% 
  filter(c_loss_ratio == c_loss_selected) %>% 
  select(Region = region, Scenario = scenario, `Land-Use Change` = luc_co2_high, `Input Use` = tot_emiss, `Total` = tot_co2_high) %>% 
