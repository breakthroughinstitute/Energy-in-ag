...
var_models = tibble(var = c("en_emiss_ha", "fert_emiss_ha", "fert_m_emiss_ha", "m_emiss_ha", "pest_emiss_ha", "tot_emiss_ha"),
                   model = c("en_emiss_ols", "fert_emiss_ols", "fert_m_emiss_ols", "m_emiss_ols", "pest_emiss_ols", "tot_emiss_ols"))
...
reg_fe_results <- reg_fe_results %>% 
  pivot_longer(c(fit, lwr, upr, se_log), names_to = "estimate") %>% #make dataframe long
...
region_totals <- reg_fe_results %>% 
  ...
  filter(!estimate == "se_log") %>% #filter to exclude logged standard errors
...
fig_dat <- all_emiss %>% 
  ...
  mutate(scenario = recode(scenario, "b" = "Baseline", "p" = "Projected", "h" = "High"),#Spell out scenarios for graphing 
         scenario = factor(scenario, levels = c("Baseline", "Projected", "High"))) %>%  #change order of scenario column
...
```{r Fig . Emissions by Input Type by Region by Scenario}
#column chart with error bars showing each type of emissions (energy, fertilizer, pesticide, machinery, luc), by scenario and region
ggplot(fig_dat) +
  geom_col(aes(x = scenario, y = fit, fill = source), position = "dodge") +
  geom_errorbar(aes(x = scenario, ymin = lwr, ymax = upr, fill=source), width = 0.2, position = position_dodge(.9)) +
...
figS1 <- fig_dat %>% 
  ...
  scale_fill_manual(values = setNames(c(bti_colors[1], bti_colors[2]), c("tot_emiss", luc_scenario_included)), #set colors
                     labels = setNames(c("Manufacturing and Use of Farm Inputs", "Land Use Change"), c("tot_emiss", luc_scenario_included)))+ #set labels
  theme_bti() + 
  geom_hline(yintercept = 0, color = "black", linewidth = 0.5) #add x axis line since doesnt show up for all when facetted
