...
#total kcal and cropland area across all crops by country & year. 1961-2020
...
# #Barbados, Kuwait, Belgium, Martinque occupy many of the highest values, over 20 mill. Ireland has some too, especially after ~2011. China, mainland; Germany; Netherlands, and several other ciountries have similarly high values above 18 or 19 million. Given the variety of countries and the unknown reason why these are so high, we leave them in. The results are negligibly different than if we omit values greater than 20 million.
...
filter_reg <- reg_all_countries #filter(reg_all_countries, reg_all_countries$yield < 20000000) can comment in to omit particularly high yield values
...
```{r Unused now Calculate predicted input emissions and use per ha by input, scenario and region}
...
```{r Unused now Calculate total input emissions for each input source and region}
...
scenarios_predictions <- scenarios_predictions %>% 
  ...
  pivot_longer(cols = -c(country, scenario, harv_area, yield, luc, arable_area, production), names_to = c(".value", "estimate"), names_pattern = "(.*)_(.*)") %>% 
  mutate(
...
```{r Compare linear and log regression models}

#energy e_lin_r =   e_log_r = e_lin_qq = 
