...
df_agg_conv <- naniar::replace_with_na_all(data = df_agg, condition = ~.x == 0)
```

```{r Save regresion input data}
#aggregated data used in regression
write_csv(df_agg_conv %>% select(country, year, cropland, en_emiss_ha, p_ha, fert_emiss_ha, m_energy_ha, yield), "int_data/input_data_cleaned_converted_aggregated.csv")
