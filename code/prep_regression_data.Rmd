...
```{r Save non-aggregated regresion input data}
write_csv(df_conv %>% select(country, year, cropland, en_emiss_ha, p_ha, fert_emiss_ha, m_energy_ha, yield), "int_data/input_data_cleaned_converted.csv")
```
...
Africa[which(!Africa %in% unique(df_conv$country))]
...
df_conv$country <- recode(df_conv$country, "Côte d'Ivoire"="Côte d’Ivoire")

#create new column with names as factors to revise
df_conv <- mutate(df_conv, Country_rename = country)
...
df_conv$Country_rename[df_conv$Country_rename %in% E_Afr & df_conv$country %in% africa_scenario_countries] <-  "E_Afr"
df_conv$Country_rename[df_conv$Country_rename %in% W_Afr & df_conv$country %in% africa_scenario_countries] <-  "W_Afr"
df_conv$Country_rename[df_conv$Country_rename %in% C_Afr & df_conv$country %in% africa_scenario_countries] <-  "C_Afr"
df_conv$Country_rename[df_conv$Country_rename %in% S_Afr & df_conv$country %in% africa_scenario_countries] <-  "S_Afr"
df_conv$Country_rename[df_conv$Country_rename %in% N_Afr & df_conv$country %in% africa_scenario_countries] <-  "N_Afr"
```
...
reg_E <- dplyr::filter(df_conv, Country_rename == "E_Afr" )
reg_W <- dplyr::filter(df_conv, Country_rename == "W_Afr" )
reg_C <- dplyr::filter(df_conv, Country_rename == "C_Afr" )
reg_S <- dplyr::filter(df_conv, Country_rename == "S_Afr" )
reg_N <- dplyr::filter(df_conv, Country_rename == "N_Afr" )
...
df_other_countries <- df_conv %>% filter(!Country_rename %in% c("N_Afr", "C_Afr", "S_Afr","E_Afr","W_Afr")) %>% select(-Country_rename)
...
```{r Explore data and remove outlier yield values}
#Make sure any newly created 0's are converted to NAs, lm function automatically removes data with NAs 
df_agg <- naniar::replace_with_na_all(data = df_agg, condition = ~.x == 0)
```

```{r Save aggregated regresion input data}
write_csv(df_agg %>% select(country, year, cropland, en_emiss_ha, p_ha, fert_emiss_ha, m_energy_ha, yield), "int_data/input_data_cleaned_converted_aggregated.csv")
```
